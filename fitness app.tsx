...
function HomeScreen({ sessions, onAddSession, onDeleteSession, onEditSession, onStart, onAddWorkout, profile, wks }) {
  ...
  const upNext = (() => {
    const list = Object.values(wks).map(wk => {
      const last = sessions
  ...
  return (
    <div style={{padding:"18px 18px 16px",overflowY:"auto"}}>
      ...
      <Card className="fu" style={{padding:"11px 16px 9px",marginBottom:10,animationDelay:"0.06s"}}>
        ...
        <div style={{display:"flex",gap:8}}>
          <GoalRing value={wkStrengthN} target={3} col={ACT.strength.col} label="Strength" />
          <GoalRing value={wkRunsN}     target={2} col={ACT.run.col}      label="Runs" />
          <GoalRing value={wkCoreN}     target={2} col={ACT.core.col}     label="Mobility" />
        </div>
      </Card>
      ...
      {tab === "anyone" ? (
        ...
        <div>
          <div style={{fontSize:12.5,color:C.inkMid,lineHeight:1.6,marginBottom:14}}>
            Send this code to another Cadence user. They paste it into
            <strong> Add {"\u2192"} Build {"\u2192"} paste code</strong> to get an exact copy of
            this workout, ready to use.
          </div>
