...
function applyTheme(dark) {
  if (applyTheme._cur === dark) return;   // no-op if unchanged — avoids mid-render churn
  applyTheme._cur = dark;
...
function CompletionScreen({ workout, elapsed, done, total, pbs, onDone }) {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:C.canvas,zIndex:300,
      ...
      <div className="pi" style={{fontSize:72,marginBottom:16,lineHeight:1}}>
        {workout.emoji}
      </div>
...
function WorkoutScreen({ workoutId, onFinish, onBack, wks, history }) {
  ...
  function toggleSet(key) {
    ...
    if (!wasChecked && restDur > 0) setRestLeft(restDur);  // start rest on completion
  }
  ...
  function handleSave() {
    ...
    onFinish({
      ...
      durationSecs:elapsed, note, weights, pbs,
    });
  }
                  ...
                  <Card key={ex.gi} style={{marginBottom:8,padding:"13px 14px",
                    ...
                    <div style={{display:"flex",justifyContent:"space-between",
                      ...
                      {ex.weight !== null && (
                        <div style={{display:"flex",alignItems:"center",gap:6,
                          ...
                          <button onClick={() => setWeights(ws => ({...ws,[ex.gi]:Math.max(0,wv-2.5)}))}
                          ...
                          <button onClick={() => setWeights(ws => ({...ws,[ex.gi]:wv+2.5}))}
                            style={{background:"none",border:"none",cursor:"pointer",
                              color:C.inkSoft,fontSize:18,lineHeight:1,padding:0}}>
                            +
                          </button>
                    ...
                    <div style={{display:"flex",gap:7,flexWrap:"wrap"}}>
                      {Array.from({length:ex.sets}).map((_, si2) => {
                        const key = ex.gi + "-" + si2;
                        return (
                          <SetBtn key={si2} num={si2+1} done={!!checked[key]}
                            hue={ac}
    ...
    <Sheet open={open} onClose={handleClose}
      ...
      {saved ? (
              ...
              <div style={{display:"flex",gap:7,flexWrap:"wrap",alignItems:"flex-end"}}>
                ...
                {ex.weightMode === "weight" ? (
                  <span style={{display:"flex",alignItems:"flex-end",gap:4}}>
                    <MiniField label="Weight" value={ex.weight} onChange={v => updateEx(i,"weight",v)} w={56} suffix="kg" />
        ...
        <div>
          ...
          <div style={{display:"flex",gap:6,marginBottom:16,marginTop:-6}}>
            {["💪","🏋️","🔥","🦵","🧘","🏃"].map(e => (
              <button key={e} onClick={() => setEmoji(e)}
                style={{width:38,height:38,borderRadius:11,fontSize:19,cursor:"pointer",
                  background:emoji === e ? C.sageFog : C.surfaceAlt,
                  border:"1.5px solid " + (emoji === e ? C.sage : C.line)}}>{e}</button>
            ))}
          </div>
          ...
          <div style={{display:"flex",gap:6,marginBottom:14,marginTop:-6}}>
            {["💪","🏋️","🔥","🦵","🧘","🏃"].map(e => (
              <button key={e} onClick={() => setEmoji(e)}
                style={{width:38,height:38,borderRadius:11,fontSize:19,cursor:"pointer",
                  background:emoji === e ? C.sageFog : C.surfaceAlt,
                  border:"1.5px solid " + (emoji === e ? C.sage : C.line)}}>
                {e}
              </button>
            ))}
          </div>
...
function TrainScreen({ onStart, sessions, onAddWorkout, onDeleteWorkout, wks, hiddenCount, onRestore, onLogSession }) {
  ...
  return (
    <div style={{padding:"28px 20px 28px",overflowY:"auto"}}>
      ...
      <WorkoutBuilderSheet open={!!editing} onClose={() => setEditing(null)}
  ...
  const volumeWeeks = (() => {
    ...
    workouts.forEach(s => {
      ...
      eachExercise(def, (ex, gi) => {
        ...
        if (reps && w && w > 0) vol += (ex.sets || 1) * reps * w;
      });
...
function SettingsScreen({ profile, setProfile, themeMode, setThemeMode,
...
============================================================ */
/* ============================================================
   ONBOARDING  (#1) — brief first-run welcome for new users
============================================================ */
function Onboarding({ onDone }) {
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:400,background:C.canvas,
      ...
      <div style={{margin:"auto 0",paddingTop:48,paddingBottom:32,width:"100%",maxWidth:380,marginLeft:"auto",marginRight:"auto"}}>
        <div style={{fontFamily:FONT.display,fontSize:42,lineHeight:1.05,marginBottom:8}}>
          Cadence
        </div>
        <div style={{fontSize:14,color:C.inkMid,lineHeight:1.55,marginBottom:30}}>
          A calm place to plan your workouts, log what you do, and watch the weeks add up.
          Everything stays on your device.
        </div>

        <div style={{fontSize:11,fontWeight:600,color:C.inkSoft,textTransform:"uppercase",
          letterSpacing:"0.08em",marginBottom:8}}>What should we call you?</div>
        <input value={name} onChange={e => setName(e.target.value)}
          placeholder="First name (optional)"
          style={{width:"100%",background:C.surface,border:"1.5px solid " + C.line,
            borderRadius:12,padding:"13px 15px",fontSize:15,color:C.ink,fontFamily:FONT.ui,
            outline:"none",boxSizing:"border-box",marginBottom:22}} />

        <div style={{fontSize:11,fontWeight:600,color:C.inkSoft,textTransform:"uppercase",
          letterSpacing:"0.08em",marginBottom:8}}>Units</div>
        <div style={{display:"flex",gap:6,background:C.surfaceAlt,borderRadius:12,padding:4,marginBottom:30}}>
          {[["metric","Metric (kg, km)"],["imperial","Imperial (lb, mi)"]].map(([u, lbl]) => (
        ...
        <button onClick={() => { haptic(12); onDone({ firstName: name.trim(), units }); }}
          style={{width:"100%",padding:"15px 0",border:"none",cursor:"pointer",borderRadius:14,
            ...
            boxShadow:"0 3px 12px " + C.sage + "66",marginBottom:14}}>
          Get started
        </button>
        <div style={{fontSize:11,color:C.inkMuted,textAlign:"center",lineHeight:1.5}}>
...
export default function App() {
  ...
  applyTheme(isDark);
  useEffect(() => { applyTheme(isDark); forceRender(n => n + 1); }, [isDark]);
        ...
        {incomingWk && (
          <div>
            <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14}}>
              <div style={{fontSize:34,lineHeight:1}}>{incomingWk.emoji || "💪"}</div>
              <div>
                <div style={{fontFamily:FONT.display,fontSize:20,color:C.ink,lineHeight:1.1}}>
                  {incomingWk.label}
                </div>
