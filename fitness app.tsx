...
function Sheet({ open, onClose, title, children }) {
  ...
  const atTop = useRef(true);               // was the scroll area at the top when the drag began?
  const scroller = useRef(null);
  const CLOSE_AT = 100;                       // drag past this (px) to dismiss
  ...
  function onStart(e) {
    ...
    // A downward drag only dismisses when the content is scrolled to the very top;
    // otherwise the gesture scrolls the content as normal.
    atTop.current = !scroller.current || scroller.current.scrollTop <= 0;
  }
  function onMove(e) {
    if (!dragging.current || startY.current == null) return;
    const delta = clientY(e) - startY.current;
    if (delta > 0 && atTop.current) {
      setDy(delta);                           // dragging the sheet down to dismiss
    } else if (dy !== 0) {
      setDy(0);                               // let the content scroll instead
    }
  }
  function onEnd() {
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:200}}>
      ...
      <div ref={scroller}
        ...
        padding:"0 22px 44px",maxHeight:"90vh",overflowY:"auto",overscrollBehaviorY:"contain",
        ...
        animation: "slideUp 0.3s cubic-bezier(.32,1.1,.5,1)"}}>
        {/* Grab handle + title. The whole sheet is draggable; this just signals it. */}
          ...
          return (
            <div key={si} style={{marginBottom:8}}>
              ...
              {open && vis.map(ex => {
                ...
                return (
                  <SwipeToDelete key={ex.gi} onDelete={() => removeExercise(ex.gi)}>
    ...
    <Sheet open={open} onClose={handleClose}
      ...
      {saved ? (
        ...
        <div>
          ...
          {picked.map((ex, i) => (
            <SwipeToDelete key={i} onDelete={() => removeEx(i)}>
          ...
          {secs.map((sec, si) => (
            <div key={si} style={{marginBottom:14}}>
              ...
              {sec.exercises.map((ex, xi) => (
                <SwipeToDelete key={xi} onDelete={() => removeEx(si, xi)} marginBottom={7}>
                <Card style={{marginBottom:0,padding:"10px 11px"}}>
