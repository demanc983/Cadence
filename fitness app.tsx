...
const EXERCISE_LIBRARY = [
  ...
  { name:"Shoulder Taps", equip:"bodyweight", muscle:"Core" },
...
/* Swipe-left-to-delete wrapper. Drags horizontally with the finger, reveals a red
   Delete backing, and removes the row when released past the threshold. Vertical
   scrolling is preserved by locking to the dominant axis on the first move. */
function SwipeRow({ onDelete, children, radius = 14, mb = 8 }) {
  const [dx, setDx] = useState(0);
  const [dragging, setDragging] = useState(false);
  const st = useRef(null);
  const THRESH = 78;
  function ts(e) { const t = e.touches[0]; st.current = { x: t.clientX, y: t.clientY, lock: null }; }
  function tm(e) {
    if (!st.current) return;
    const t = e.touches[0], dX = t.clientX - st.current.x, dY = t.clientY - st.current.y;
    if (st.current.lock == null && (Math.abs(dX) > 8 || Math.abs(dY) > 8))
      st.current.lock = Math.abs(dX) > Math.abs(dY) ? "h" : "v";
    if (st.current.lock !== "h") return;
    if (!dragging) setDragging(true);
    setDx(Math.max(-130, Math.min(0, dX)));
  }
  function te() {
    if (!st.current) return;
    const past = dx <= -THRESH;
    st.current = null; setDragging(false);
    if (past) { setDx(-520); haptic(16); setTimeout(() => { onDelete && onDelete(); }, 170); }
    else setDx(0);
  }
  return (
    <div style={{position:"relative",overflow:"hidden",borderRadius:radius,marginBottom:mb}}>
      <div style={{position:"absolute",inset:0,background:C.terracotta,borderRadius:radius,
        display:"flex",alignItems:"center",justifyContent:"flex-end",gap:6,paddingRight:18,
        color:C.white,fontFamily:FONT.ui,fontSize:13,fontWeight:700,
        opacity:dx < -8 ? 1 : 0,transition:"opacity 0.15s"}}>
        <Ic n="trash" sz={16} col={C.white} />
        Delete
      </div>
...
function WorkoutScreen({ workoutId, onFinish, onBack, wks, history }) {
  ...
  const [removed, setRemoved] = useState({});   // gi -> true: removed for this session
  const removeExercise = (gi) => { haptic(14); setRemoved(r => ({ ...r, [gi]: true })); };
  let cur = 0;
  const sections = w.sections.map(sec => ({
    ...sec,
    exercises: sec.exercises.map(ex => ({ ...ex, gi: cur++ })),
  }));
  const allEx    = sections.flatMap(s => s.exercises).filter(e => !removed[e.gi]);
  ...
  // Manual rest: start a countdown now using the chosen length (falls back to 60s).
  const startRest = () => { setRestLeft(restDur > 0 ? restDur : 60); haptic(8); };
  ...
  // countdown ONLY when this exercise has its own built-in Rest value. Otherwise no
  // automatic rest — the user starts one with the Rest button when they want it.
  function toggleSet(key, exRest, target) {
    ...
    if (!wasChecked) {
      ...
      if (exRest != null && +exRest > 0) setRestLeft(+exRest);
    }
  }

  const doneSets = allEx.reduce((a, e) =>
    a + Array.from({ length: e.sets }).filter((_, i) => checked[e.gi + "-" + i]).length, 0);
    ...
    <div style={{display:"flex",flexDirection:"column",height:"100%",background:C.canvas}}>
      ...
      <div style={{background:C.surface,padding:"16px 20px 14px",
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:5}}>
          <span style={{fontSize:10,color:C.inkMuted}}>
            {doneSets} of {totalSets} sets · {Math.round(pct*100)}%
          </span>
          <div style={{display:"flex",alignItems:"center",gap:7}}>
            <button onClick={cycleRest} aria-label="Rest length"
              style={{background:"transparent",border:"1px solid " + C.line,
                borderRadius:100,padding:"3px 9px",cursor:"pointer",fontFamily:FONT.ui,
                fontSize:9.5,fontWeight:600,color:C.inkMuted}}>
              {restDur ? restDur + "s" : "off"}
            </button>
            <button onClick={startRest}
              style={{background:C.sage,border:"none",borderRadius:100,padding:"4px 12px",
                cursor:"pointer",fontFamily:FONT.ui,fontSize:10,fontWeight:700,color:C.white,
                display:"flex",alignItems:"center",gap:4}}>
              <Ic n="timer" sz={11} col={C.white} />
              Rest
            </button>
          </div>
        </div>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"14px 16px 0"}}>
        {sections.map((sec, si) => {
          ...
          const vis = sec.exercises.filter(ex => !removed[ex.gi]);
          if (vis.length === 0) return null;   // hide a section once all its exercises are removed
          return (
            <div key={si} style={{marginBottom:8}}>
              ...
              {open && vis.map(ex => {
                ...
                return (
                  <SwipeRow key={ex.gi} onDelete={() => removeExercise(ex.gi)}>
                  <Card style={{marginBottom:0,padding:"13px 14px",
    ...
    <Sheet open={open} onClose={handleClose}
      ...
      {saved ? (
        ...
        <div>
          ...
          {picked.map((ex, i) => (
            <SwipeRow key={i} onDelete={() => removeEx(i)}>
            <Card style={{padding:"12px 13px",marginBottom:0}}>
...
function WorkoutEditorSheet({ open, onClose, editWorkout, onSaveWorkout, onDeleteWorkout }) {
    ...
    if (open && editWorkout && seeded !== editWorkout.id) {
      ...
      setSecs((editWorkout.sections || []).map(s => ({
        ...
        exercises: (s.exercises || []).map(e => {
          const t = parseTimed(e.reps);
          return {
            name: e.name || "",
            sets: e.sets != null ? String(e.sets) : "3",
            timed: t.timed,                       // time-based hold (#time)
            time: t.timed ? t.time : "30",
            each: t.each,                          // "side"/"leg"/"arm" qualifier, if any
  ...
  function onPick(ex) {
    ...
    if (!pick) return;
    if (pick.mode === "add") {
      const ne = { name: ex.name, sets: "3",
        timed: !!ex.timed, time: "30", each: "",
        reps: ex.timed ? "30 sec" : "10",
    ...
    const clean = secs
      ...
      .map(s => ({
        ...
        exercises: s.exercises.map(e => ({
          ...
          reps: e.timed
            ? ((parseInt(e.time, 10) > 0 ? parseInt(e.time, 10) : 30) + " sec" + (e.each ? " / " + e.each : ""))
            : ((e.reps == null || e.reps === "") ? "-" : String(e.reps).trim()),
              ...
              {sec.exercises.map((ex, xi) => (
                <Card key={xi} style={{marginBottom:7,padding:"10px 11px"}}>
                  ...
                  <div style={{display:"flex",gap:7,alignItems:"flex-end",flexWrap:"wrap"}}>
                    ...
                    {ex.timed ? (
                      <div>{fldLbl("Time (s)")}{numIn(ex.time, v => setExField(si, xi, "time", v), 58, "30")}</div>
                    ) : (
                      <div>{fldLbl("Reps")}
                        <input value={ex.reps} onChange={e => setExField(si, xi, "reps", e.target.value)}
                          placeholder="-"
                          style={{width:72,background:C.surfaceAlt,border:"1px solid " + C.line,borderRadius:8,
                            padding:"7px 8px",fontSize:13,color:C.ink,outline:"none",fontFamily:FONT.ui,
                            boxSizing:"border-box",textAlign:"center"}} />
                      </div>
                    )}
                    <div>{fldLbl("\u00A0")}
                      <button onClick={() => setExField(si, xi, "timed", !ex.timed)}
                        style={{height:33,padding:"0 9px",borderRadius:8,cursor:"pointer",
                          border:"1px solid " + C.sageLight,background:C.sageFog,color:C.sageDark,
                          fontFamily:FONT.ui,fontSize:11,fontWeight:600,whiteSpace:"nowrap"}}>
                        {ex.timed ? "\u21BA Reps" : "\u21BA Time"}
                      </button>
                    </div>
...
const DEFAULT_GOALS = { strength: 3, runs: 2, walks: 2, mobility: 2 };
function getGoals(profile) {
  ...
  return {
    ...
    walks:    Math.max(1, Math.min(14, +g.walks    || DEFAULT_GOALS.walks)),
...
function HomeScreen({ sessions, onAddSession, onDeleteSession, onEditSession, onStart, onAddWorkout, profile, wks }) {
  ...
  const wkWalksN = wkSess.filter(s =>
    s.type === "run" && /walk/i.test(s.label || "")).length;
    ...
    <div style={{padding:"18px 18px 16px",overflowY:"auto"}}>
      ...
      <Card className="fu" style={{padding:"11px 16px 9px",marginBottom:10,animationDelay:"0.06s"}}>
        ...
        <div style={{display:"flex",gap:8}}>
          ...
          <GoalRing value={wkWalksN}    target={goals.walks}    col={ACT.walk.col}     label="Walks" />
...
// Detect a time-based prescription ("30 sec", "20-30 sec / side") and pull out the
// duration (upper bound of a range) and any per-side/leg qualifier.
function parseTimed(reps) {
  const s = String(reps || "");
  if (!/\bsec\b/i.test(s)) return { timed: false, time: "", each: "" };
  const nums = s.match(/\d+/g) || [];
  const time = nums.length ? nums[nums.length - 1] : "30";
  const each = /\/\s*side/i.test(s) ? "side" : /\/\s*leg/i.test(s) ? "leg"
             : /\/\s*arm/i.test(s) ? "arm" : "";
  return { timed: true, time, each };
}
...
function SettingsScreen({ profile, setProfile, themeMode, setThemeMode, scheme, setScheme,
  ...
  return (
    <div style={{padding:"28px 20px 36px",overflowY:"auto",height:"100%"}}>
      ...
      {[["strength","Strength sessions"],["runs","Runs"],["walks","Walks"],["mobility","Mobility / core"]].map(([k, lbl]) => {
...
function Onboarding({ onDone, scheme, setScheme }) {
  ...
  const [goals,  setGoals]  = useState({ ...DEFAULT_GOALS });  // weekly targets
  ...
  function buildPatch() {
    ...
    return {
      firstName: name.trim(), units, goals,
      ...(wKg ? { weightKg: wKg } : {}),
      ...(heightCm ? { heightCm } : {}),
    };
  }
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:400,background:C.canvas,
      ...
      <div style={{margin:"auto 0",paddingTop:44,paddingBottom:32,width:"100%",maxWidth:380,
        ...
        {lbl("Weekly goals")}
        <div style={{fontSize:11.5,color:C.inkMuted,lineHeight:1.5,marginTop:-3,marginBottom:10}}>
          How many of each you'd like to aim for each week. These set your Home rings — change them anytime.
        </div>
        <div style={{marginBottom:30}}>
          {[["strength","Strength"],["runs","Runs"],["walks","Walks"],["mobility","Mobility"]].map(([k, l]) => (
            <div key={k} style={{display:"flex",alignItems:"center",justifyContent:"space-between",
              padding:"9px 0",borderBottom:"1px solid " + C.lineSoft}}>
              <span style={{fontSize:13.5,color:C.ink}}>{l}</span>
              <div style={{display:"flex",alignItems:"center",gap:11}}>
                <button onClick={() => { haptic(6); setGoals(g => ({ ...g, [k]: Math.max(1, (g[k] || 1) - 1) })); }}
                  style={{width:34,height:34,borderRadius:10,border:"1.5px solid " + C.line,
                    background:C.surfaceAlt,cursor:"pointer",fontSize:18,fontWeight:600,color:C.inkMid,
                    lineHeight:1,display:"flex",alignItems:"center",justifyContent:"center"}}>{"\u2212"}</button>
                <span style={{fontFamily:FONT.display,fontSize:19,color:C.ink,minWidth:22,textAlign:"center"}}>{goals[k]}</span>
                <button onClick={() => { haptic(6); setGoals(g => ({ ...g, [k]: Math.min(14, (g[k] || 1) + 1) })); }}
                  style={{width:34,height:34,borderRadius:10,border:"1.5px solid " + C.line,
                    background:C.surfaceAlt,cursor:"pointer",fontSize:18,fontWeight:600,color:C.inkMid,
                    lineHeight:1,display:"flex",alignItems:"center",justifyContent:"center"}}>+</button>
                <span style={{fontSize:10.5,color:C.inkMuted,width:26}}>/ wk</span>
              </div>
            </div>
          ))}
        </div>
